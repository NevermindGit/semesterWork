...
app.config['UPLOAD_FOLDER'] = '/Users/vadimvaleev/Downloads/semesterWork/static/images/'
...
def get_productparam_id(pid):
    conn = get_db_connection()
    product = conn.execute('SELECT * FROM product_parameters WHERE p_id = ?', (pid,)).fetchone()
    conn.close()
    if product is None:
        abort(404)
    return product
    ...
    if request.method == 'POST':
        ...
        if not name:
            ...
            if user_id == product['u_id']:
                ...
                conn.execute('UPDATE product_parameters SET size = ?, maker = ? WHERE p_id = ?', (size, maker, product['product_id']))
                conn.commit()
                conn.close()
                return redirect(url_for('products_list'))
    ...
    if request.method == 'POST':
        ...
        if not name:
            ...
            if log:
                ...
                insert(table_name='product_parameters', p_id=product_id, size=size, maker=maker)
                conn.commit()
                conn.close()
                return redirect(url_for('products_list'))
...
def insert(table_name, **kwargs):
    conn = get_db_connection()
    arg_v = ', '.join(["?"] * (len(kwargs)))
    arg_names = ",".join(kwargs.keys())

    arg_values = tuple(kwargs.values())
    sql_raw = f"insert into {table_name} ({arg_names}) values ({arg_v})"
    conn.execute(sql_raw, arg_values)
    conn.commit()
    conn.close()


@app.route('/<int:p_id>/addToBasket')
def addToBasket(p_id):
    if session['name']:
        insert(table_name='basket', user_id=get_id(), pic_id=p_id)
        return redirect(url_for('products_list'))
    else:
...
def basket():
    if session['name']:
        conn = get_db_connection()
        user_id = get_id()
        products = conn.execute('SELECT * FROM products JOIN basket ON pic_id=product_id  WHERE user_id=?',(user_id,)).fetchall()
        return render_template('basket.html', products=products)
    return redirect(url_for('signup'))


@app.route('/<int:p_id>/removefrombasket')
def removefrombasket(p_id):
    conn = get_db_connection()
    conn.execute("DELETE FROM basket WHERE id = ?",(p_id,))
    conn.commit()
    conn.close()
